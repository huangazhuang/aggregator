...
这套流水线不会修改现有的 `clash-verge-output`。它从该分支读取完整订阅，并维护两个彼此隔离的 CNB 输出：现有 `clash-cn-output` 继续使用 gstatic 做 20 轮选拔，作为当前默认和备用订阅；新增的 `clash-cn-gmgn-output` 使用 GMGN 做 20 轮严格对齐测速，专门帮助用户优先手动测速和选择节点。GMGN 输出在连续观察 2–3 轮前不会替换现有默认链接，任一任务失败也不会互相覆盖。
...
```text
https://cnb.cool/<你的组织>/<仓库>/-/git/raw/clash-cn-gmgn-output/clash.yaml
```

GMGN 订阅是新增的观察通道，不是对默认 gstatic 订阅的原地替换。建议先把两个链接分别导入 Clash Verge，对比连续 2–3 次 CNB 运行和本机手动测速结果后，再决定是否切换日常使用链接。
...
4. 再次向 GitHub `main` 推送提交，或在 GitHub Actions 中手动运行 `Sync GitHub main to CNB`。手动运行时必须在分支选择器中选 `main`；工作流也会硬性拒绝其他分支。把 `trigger_probe` 设为 `true`，会先确认 CNB `main` 已同步到同一提交，再向 CNB 单独推送一个唯一的 `cnb-probe-*` 标签，由受信任的 `tag_push` 事件启动同一套大陆探测。若要运行 GMGN 四分片测速，则把 `trigger_gmgn_shadow` 设为 `true`；它会推送独立的 `cnb-gmgn-shadow-*` 标签，同时刷新脱敏影子报告和独立的 `clash-cn-gmgn-output` 优先手测订阅，不会改动 `clash-cn-output`。标签只存在于 CNB、用于追踪手动运行，不会写入 GitHub；令牌只从 GitHub Secret 注入，不写入日志。
...
- 控制组和金丝雀通过与节点测速相同的固定 Mihomo `DIRECT` delay API 发起请求，避免 Python TLS/HTTP 指纹与节点测速客户端不同而产生假阴性；失败日志只输出分片成功计数、连续失败数和脱敏错误类别；
...
影子报告会继续保留，用于确认 GMGN 实际分布、是否存在后半程整体成功率下降、四分片性能和门槛容量。它不包含可还原的节点凭据，也不是 Clash 订阅。

### GMGN 正式选拔规则

- 所有源节点都完整测试 20 轮，不设会因预检或单次 timeout 提前淘汰节点的候选截断。
- 每次请求最多等待 3000 ms，以便区分慢响应和无响应；只有实际延迟不高于 1000 ms 的轮次才算达标，与 Clash Verge 中使用的 1000 ms timeout 标准对齐。
- 同一 CNB Runner 使用 4 个独立 Mihomo 分片，每片 16 个并发线程，总并发 64；节点之间并行，同一节点的 20 轮保持顺序。
- 亚洲核心节点要求 20 轮中至少 14 轮达标，并且前 10 轮、后 10 轮各至少达标 5 次，避免只在短时间窗口偶然可用的节点进入稳定组。
- 亚洲弹性节点为 20 轮中达标 10–13 次的节点，单独放入 `亚洲弹性` 组，不与核心稳定节点混在一起。
- 非亚洲先选择最多 10 个至少达标 16/20 的节点；只有后续候选达到至少 18/20 时，才允许再扩展最多 10 个，因此非亚洲总量仍不超过 20 个。
- 使用一次迟滞降低亚洲节点因单轮波动而消失的概率：上一版的亚洲核心如果本轮降到 12–13 次，可额外保留一个发布周期；本轮少于 10 次时不保留。迟滞节点不会永久绕过正式门槛。
- 排名依次优先达标次数、有效响应次数，再比较 P90、中位延迟和抖动；最低延迟不能掩盖频繁 timeout。
- 80 个只是期望容量，不是发布硬下限。结果只发布真实达到规则的节点，不会为了凑到 80 个加入 20 轮全失败节点；总上限为 150 个，来源质量提高时可自行扩展到 80 以上。
- 覆盖旧 GMGN 订阅前执行独立故障保护：本轮至少需要选出 `max(10, 上一版发布数量 × 40%)` 个节点，否则拒绝覆盖，继续保留最后一版可用的 `clash-cn-gmgn-output/clash.yaml`。该保护不会影响 gstatic 订阅。

第一轮真实影子数据表明，按上述门槛预计只能稳定发布约 20–22 个节点。这是当前源质量下的实际容量，不是固定上限；后续 GitHub 收集到更多高质量亚洲节点后，GMGN 输出会按相同规则自然增加，最多 150 个。

生成的 Clash 配置以手动选择为主：

- `手动选择`：包含本轮发布的全部节点，方便在 Clash Verge 中即时测速后自行选择；
- `GMGN稳定`：包含亚洲核心和通过严格门槛的非亚洲节点；
- `亚洲弹性`：包含本轮 10–13/20 的普通亚洲候选，供手动复测，不冒充稳定节点；
- `GMGN观察保留`：单独标出上一版核心、但本轮只达到 12–13/20 且仅获一次迟滞机会的节点；
- `GMGN自动`：自动测速组，只在希望自动选择时使用；`手动选择` 的首项仍是 `GMGN稳定`，不强制采用自动模式。

当前 `clash-cn-output` 继续作为 gstatic 默认和备用订阅。至少连续观察 2–3 轮 GMGN 输出、CNB 日志和 Clash Verge 本机手测结果后，再决定是否替换默认链接。
...
- 达到 90% 成功率且 P90 不高于 2000 ms 的高质量节点可以继续扩容。若存在尚未选中的精英亚洲节点，会优先用于替换基础配置中的较弱亚洲兜底；随后只有精英节点可以新增名额，兜底节点自身永远不能把配置扩到 80 以上。最终数量按真实质量决定，最多 150 个，非亚洲仍然最多 20 个。
- 动态 `required_count` 只承担 last-good 防缩水职责，不负责把结果补到 80。首次发布绝对下限为 1；后续运行取 `max(1, 上一版基础容量 × 40%)`，低于门槛时拒绝覆盖并保留上一版。
- 上一版发布量最高只按基础目标 80 计算保留比例，因此即使曾扩容到 150，下一轮保护门槛也不会超过 32；质量阈值本身不随容量降低。
...
- `MIN_SUCCESS`：允许覆盖旧订阅所需的绝对最少节点数，默认 1。它只防止空结果发布，不会要求为了达到 80 而放宽质量线。
- `MIN_RETAIN_RATIO`：相对上一版基础目标内节点数的最低保留比例，默认 0.40；超过 80 的精英扩容不会抬高下一轮硬门槛。
...
测试覆盖 3+17 轮采样、成功率/P90/抖动统计、生产使用的 `asia_tiering=True` 分级路径、亚洲优先与非亚洲 10–20 上限、80 个软目标与 150 个硬上限、last-good 防缩水门禁、脱敏回放、代理组过滤、REALITY 字符串序列化和 TCP 探针错误分类。
