...
这套流水线不会修改现有的 `clash-verge-output`。它从该分支读取完整订阅，在 CNB 云端用 Mihomo 做两阶段、最多 20 轮的协议级实测，再按成功率、P90 延迟、中位延迟和抖动筛出稳定节点，发布到 CNB 仓库的 `clash-cn-output` 分支。最终结果以亚洲节点为主，非亚洲节点只保留最稳定的一小部分。
...
- 所有节点先测 3 轮；初筛约 300 个亚洲节点和 60 个非亚洲节点进入第二阶段，再补测 17 轮，总计 20 轮。这样约需 8,000–10,000 次真实请求，但不会让明显较差的全部节点都跑满 20 轮。
...
- `status.json` 保存本次汇总；`probe-results.json` 保存每轮样本、成功率、P90、中位数、抖动、候选/合格/发布状态和淘汰原因。
...
- `candidate_count`、`qualified_count`、`published_asia_count`、`published_non_asia_count`：两阶段筛选和最终地区构成。
...
- `PRELIMINARY_ROUNDS` / `TOTAL_ROUNDS`：初筛 3 轮、候选总计 20 轮。
- `CANDIDATE_LIMIT`：第二阶段最多 360 个候选；默认目标为 300 个亚洲、60 个非亚洲。
