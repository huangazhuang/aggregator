...
该订阅会对全部节点先测 3 轮，再全部补足到 20 轮，按成功率、P90、中位延迟和抖动筛选。基础发布目标为 80 个，其中亚洲通常至少 60 个；亚洲不足时只在基础配置中按 14/20、12/20、10/20 逐级兜底，非亚洲仍保持严格 14/20 且最多 20 个。只有成功率至少 90% 且 P90 不高于 2000ms 的节点才会继续扩容，最终动态发布 80–150 个。详细说明见 [`CNB_SETUP.md`](CNB_SETUP.md)。

仓库另有一个不影响订阅的 GMGN 影子任务：它在同一 CNB 国内 Runner 上用 4 个独立 Mihomo 分片对全部节点测试 20 轮，以 1000ms 作为 Clash 对齐达标线，但请求本身最多等待 3000ms以保留慢响应诊断。影子结果只发布到 `clash-cn-gmgn-shadow`，用于确定正式切换门槛；在完成首轮数据分析前，`clash-cn-output` 仍沿用现有 gstatic 选拔。
