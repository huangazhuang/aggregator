...
When an entry point is copied into a minimal container, treat the repository →
Docker build context → image filesystem → Python import graph as a cross-layer
contract:

- [ ] Search the entry point's transitive top-level package imports, not only
      files directly named in the Dockerfile
- [ ] Update both Dockerfile `COPY` rules and the dedicated `.dockerignore`
- [ ] Keep the copied dependency set minimal and explicit
- [ ] Run the actual trusted entry point inside the built image (preferably
      `--network none`); a successful `docker build` alone is not runtime proof
- [ ] Add a contract regression for exact copied paths so later imports cannot
      silently outgrow the image
